...
def predict(user_input: InputSchema):
    prediction = make_prediction(user_input.model_dump())  #model dump converts pydantic model to dict
    return OutputSchema(predicted_price=round(prediction, 2))  #rounding the prediction to 2 decimal places
